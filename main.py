...
WIDTH, HEIGHT =  1920, 1080  # (0,0) point in the screen is top left corner. 
...
class Planet:
    ...
    def __init__(self, x, y, radius, color, mass, nameP, thesun=None):# focus on moon ? / sun will rotate 
        self.x = x
        self.y = y
        self.radius = radius
        self.color = color
    ...
    def draw(self, win):   # Makes the Center of the screen, avaliable to draw
        x = (self.x - self.thesun.x) * self.SCALE + WIDTH / 2
        y = (self.y - self.thesun.y) * self.SCALE + HEIGHT / 2
...
def main():
    ...
    sun = Planet(0, 0, 30, YELLOW, 1.98892 * 10**31, nameP= "Sun")# (x, y, radius, color,  mass)
    sun.sun = True        
    sun.y_vel = 120.4 * 1000                           # x makes it further 

    mercury = Planet(1.687 * Planet.AU, 0, 8, ORANGE, mass =  3.30 * 10**23, nameP= "Mercury", thesun=sun)
    mercury.y_vel = 47.4 * 1000 
    
    venus = Planet(0.723 * Planet.AU, 0, 14, PALE_GREEN , mass =  4.8685 * 10**22, nameP= "Venus", thesun=sun)
    venus.y_vel = 35.02 * 1000

    earth = Planet(1.7 * Planet.AU, 0, 12, BLUE, mass =  5.9742 * 10**22, nameP= "Earth", thesun=sun)
    earth.y_vel = 37.783 * 1000

    mars = Planet(-1.70001 * Planet.AU, 0, 12, RED, mass =  6.39 * 10**22, nameP= "Mars", thesun=sun)
    mars.y_vel = 34.077 * 1000

    jupiter = Planet(-2 * Planet.AU, 0, 20, ORANGE, mass = 1.898 * 10**22, nameP= "Jupiter", thesun=sun)
    jupiter.y_vel = 51.1 * 1000 # y ekseninde gidicegi yüksekligi belirliyor

    saturn = Planet(-2.5 * Planet.AU, 0, 15, GRAY, mass =  5.68 * 10**22, nameP= "Saturn", thesun=sun)
    saturn.y_vel = 79.1* 1000
    
    uranus = Planet(-3 * Planet.AU, 0, 11, DARK_BLUE, mass =  8.68 * 10**23, nameP= "Uranus", thesun=sun)
    uranus.y_vel = 97.3 * 1000
    
    neptune = Planet(-3.24 * Planet.AU, 0, 9, BLUE, 10.2 * 10**23, nameP= "Neptune", thesun=sun)
    neptune.y_vel = 96.8 * 1000
